...
# EXERCISE 3: New tool for LocalExpert agent
@tool
def search_local_tips(destination: str) -> str:
    """
    Search for local tips, customs, safety info, and hidden gems at a destination.
    Returns insider knowledge that typical tourists might miss.
    """
    local_data = {
        "Iceland": {
            "customs": [
                "Tipping is not expected in Iceland - service charges are included in prices",
                "Icelanders are informal; first names are used even in professional settings",
                "Remove shoes when entering someone's home as a sign of respect",
                "The sagas and Norse mythology are deeply important to Icelandic culture",
                "Swimming pools (sundlaug) are social gathering places, not just for exercise",
            ],
            "safety": [
                "Always check road conditions at road.is before driving - F-roads require 4WD",
                "Weather changes rapidly - always carry warm layers even in summer",
                "Stay on marked paths near geothermal areas; boiling water/mud is dangerous",
                "Never approach the ocean without checking for sneaker waves at black sand beaches",
                "Emergency number is 112; 112 Iceland app works even without cell signal",
            ],
            "hidden_gems": [
                "Nauthólsvík Geothermal Beach: free entry, heated sand and sea water in Reykjavik",
                "Raufarholshellir lava tube: less crowded than tourist caves, accessible year-round",
                "Tjörnin pond in central Reykjavik: free, beautiful, 40+ bird species",
                "Bonus supermarket on Laugavegur: cheapest groceries in Reykjavik",
                "Kaffivagninn restaurant: oldest restaurant in Reykjavik, great fish soup ($12)",
                "Grótta lighthouse peninsula: free northern lights viewing spot for Reykjavik locals",
            ],
            "budget_hacks": [
                "Fill water bottles from any tap - Iceland has the world's best tap water",
                "Reykjavik Art Museum day pass ($18) covers 3 museums",
                "Many natural attractions are free: waterfalls, geysers (outside main tourist areas)",
                "Buy Skyr (Icelandic yogurt) at grocery stores - same product, fraction of cafe price",
                "Hitchhiking in Iceland is common and relatively safe in summer months",
            ]
        },
        "default": {
            "customs": [
                f"Research local tipping customs before visiting",
                f"Learn a few words of the local language - it's appreciated",
                f"Dress modestly when visiting religious sites",
            ],
            "safety": [
            ...
            "hidden_gems": [
                "Ask hotel staff for local restaurant recommendations",
                "Visit local markets for authentic experiences",
                "Explore neighborhoods away from tourist centers",
            ],
            "budget_hacks": [
                "Use local transit instead of taxis",
                "Visit free museums on discount days",
    ...
    output = f"Local Expert Tips & Hidden Gems: {destination}\n"
    output += f"{'='*60}\n"

    output += f"\n--- Cultural Customs ---\n"
    for tip in data["customs"]:
        output += f"  • {tip}\n"
    ...
    output += f"\n--- Budget Hacks from Locals ---\n"
    for hack in data["budget_hacks"]:
        output += f"  • {hack}\n"
...
def create_flight_agent(destination: str, trip_dates: str):
    ...
    return Agent(
        ...
        # EXERCISE 2 MODIFICATION: Added budget constraints to backstory.
        # The agent now prioritizes budget airlines and cost savings above all else.
        # Observation: This changes flight recommendations to favor PLAY Airlines ($349)
        # and Norse Atlantic ($389) over Icelandair ($485) or Delta ($612).
        # The BudgetAgent's calculations will reflect these lower-cost flight selections.
        backstory="You are an experienced flight specialist with deep knowledge of "
                  ...
                  "You always prioritize direct flights over connections when available at budget prices. "
                  "You have booked thousands of flights and know how to find the cheapest fares "
                  "on budget carriers like PLAY Airlines, Norse Atlantic, and Ryanair. "
...
# EXERCISE 3: New LocalExpert agent function
def create_local_expert_agent(destination: str):
    """Create the Local Expert agent with insider knowledge tools."""
    return Agent(
        role="Local Expert",
        goal=f"Share insider knowledge, hidden gems, safety information, and cultural tips for {destination} "
             f"that typical travel guides miss. Help travelers experience {destination} like a local.",
        backstory=f"You are a seasoned traveler and cultural expert who has lived in {destination} for years. "
                  f"You know all the hidden gems, local customs, safety tips, and budget hacks that guidebooks "
                  f"never mention. You help tourists avoid common mistakes and discover the authentic side of "
                  f"{destination}. You know which restaurants locals actually eat at, which attractions are "
                  f"tourist traps, and what safety precautions are essential.",
...
# EXERCISE 3: New LocalExpert task function
def create_local_expert_task(local_expert_agent, destination: str, trip_duration: str):
    """Define the local expert tips task."""
    return Task(
        description=f"Using your deep local knowledge of {destination}, provide insider tips, "
                   f"hidden gems, cultural customs, and safety information for the {trip_duration} trip. "
                   f"Include at least 3 hidden gem recommendations not found in typical guidebooks, "
                   f"essential safety tips specific to {destination}, important cultural customs to respect, "
                   f"and budget hacks that locals use. Also identify any tourist traps to avoid.",
        agent=local_expert_agent,
        expected_output=f"A comprehensive local insider guide for {destination} with hidden gems, "
                       f"cultural customs, safety tips, budget hacks from locals, "
                       f"and tourist traps to avoid for a {trip_duration} trip"
    )
